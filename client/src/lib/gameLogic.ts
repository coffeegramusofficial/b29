...
export function canAttackTarget(attacker: GameCard, target: Target, gameState: GameState): boolean {
  ...
  // Special rule for spies: they can always attack the caravan, regardless of attack value
  if (attacker.unitType === "spy") {
    return target.type === "caravan";
  }

  // For all other units, check if they have attack power
  ...
  switch (attacker.unitType) {
    ...
    case "spy":
      // Spies can ONLY attack the caravan, nothing else (already handled above)
      return target.type === "caravan";
