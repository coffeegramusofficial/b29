...
export const useGameState = create<GameStateStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    initializeGame: (whiteGuardDeckIds: string[], bolshevikDeckIds: string[], gameMode: "bot" | "multiplayer" = "bot") => {
      ...
      // Determine player faction and starting player
      let playerFaction: "white_guard" | "bolshevik";
      let startingPlayer: "white_guard" | "bolshevik";
      
      if (gameMode === "bot") {
        // In bot mode, player is always white_guard, bot is always bolshevik
        playerFaction = "white_guard";
        // Randomly determine who starts
        startingPlayer = Math.random() < 0.5 ? "white_guard" : "bolshevik";
      } else {
        // In multiplayer mode, randomly assign player faction
        playerFaction = Math.random() < 0.5 ? "white_guard" : "bolshevik";
        startingPlayer = Math.random() < 0.5 ? "white_guard" : "bolshevik";
      }
    ...
    attack: (attackerCardId: string, target: Target) => {
      ...
      // Calculate damage - spies always deal at least 1 damage, other units must have attack > 0
      const damage = attacker.unitType === "spy" ? Math.max(1, attacker.attack || 0) : (attacker.attack || 0);
      if (damage <= 0) return; // Non-spy units with no attack cannot deal damage
